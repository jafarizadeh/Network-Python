#!/usr/bin/env python3
"""
UDP Chat Server – lobby + private rooms with invites.
System messages are **only logged** on the server; they are not
broadcast to clients (except direct feedback with `_send`).
"""
...
class UDPChatServer:
    ...
    def _disconnect(self, addr: Tuple[str, int]) -> None:
        ...
        LOG.info("%s left the chat", client.name)  # system-only
    ...
    def _handle_join(self, pkt: dict, addr: Tuple[str, int]) -> None:
        ...
        LOG.info("%s joined the chat", name)  # logged only
