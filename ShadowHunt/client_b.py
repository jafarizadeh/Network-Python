...
def main(stdscr):
    ...
    while True:
        ...
        # Draw own player
        stdscr.addstr(player_y, player_x, 'B')

        # Draw other player if available
        ox, oy = other_player.get('x'), other_player.get('y')
        if ox is not None and oy is not None:
            stdscr.addstr(oy, ox, 'A')

        # Send position
        msg = json.dumps({'player': 'B', 'x': player_x, 'y': player_y})
